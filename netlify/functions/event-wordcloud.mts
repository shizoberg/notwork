...
export default async (request: Request, _context: Context) => {
  if (request.method === "GET") {
    ...
    return runWithEventRequestContext(eventIdentifier, "wordcloud", async () => {
  ...
  try {
    ...
    return await runWithEventRequestContext(eventIdentifier, "wordcloud", async () => {
      const store = getWordcloudStore();
      await ensureSeeded(store);
...
export const config: Config = {
  path: ["/api/events/21-agustos/wordcloud", "/api/event-products/wordcloud"],
};
