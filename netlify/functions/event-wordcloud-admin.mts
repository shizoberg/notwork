...
async function adminPayload(store: ReturnType<typeof getWordcloudStore>) {
  const [event, questions, answers, results] = await Promise.all([
  ...
  try {
    ...
    if (action === "updateEvent") {
      ...
      const event = {
        ...
        allowEdit:
          typeof input.event?.allowEdit === "boolean" ? input.event.allowEdit : current.allowEdit,
        updatedAt: now,
      };
      await Promise.all([
...
export const config: Config = { path: "/api/admin/events/21-agustos/wordcloud" };
